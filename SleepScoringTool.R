...
file_path1_eeg <- ""
file_path2_eeg <- "" #NOTE: If not using two eeg files, there is no need to change this line,
#but you must change the eeg_answer argument below to "One."
#EXAMPLE PATH: "/Users/jacobellen/desktop/eeg1exampledata.csv"
...
#OPTION TO RUN THE SAME MODEL ON OTHER FILES OF THE SAME RAT
#NOTE: You don't need any training data for additional files of same rat
...
scoreotherfilesofsamerat <- "No"
filepatheeg1_additionalfiles <- c("")
filepatheeg2_additionalfiles <- c("")
filepathemg_additionalfiles <- c("") 
#EXAMPLE PATH: "/Users/jacobellen/desktop/eeg1exampledata_additionalfile.csv"
...
frequency_answer <- "No"
file_output_path <- ""
#EXAMPLE PATH: "/Users/YOURNAME/desktop/spectraldata.csv"
...
if (scoreotherfilesofsamerat == "Yes") {
  print("Analyzing Additional EEG Data")
for (c in 1:(length(filepatheeg1_additionalfiles))) {
  ...
  predictions_newfile <- as.data.frame(predictions_newfile)
  
  #Finalizing Predictions and Formatting the Final Output
  predictions.output1 <- as.data.frame(cbind(eeg1,predictions_newfile))
  colnames(predictions.output1)[ncol(predictions.output1)] <- "ModelPredictions"
